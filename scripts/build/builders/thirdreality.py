...
class ThirdrealityApp(Enum):
    LIGHT = auto()

    def ExampleName(self):
        if self == ThirdrealityApp.LIGHT:
            return 'lighting-app'
        else:
...
class ThirdrealityBoard(Enum):
    ...
    def GnArgName(self):
        if self == ThirdrealityBoard.BL602_NIGHT_LIGHT:
            return 'BL602-NIGHT-LIGHT'
        elif self == ThirdrealityBoard.RELAY:
            return 'RELAY'
        elif self == ThirdrealityBoard.PLUG:
            return 'PLUG'
        elif self == ThirdrealityBoard.NL602:
            return 'NL602'
        elif self == ThirdrealityBoard.BL706_IoT_DVK:
            return 'BL706-IoT-DVK'
        else:
            raise Exception('Unknown board #: %r' % self)


class ThirdrealityBuilder(GnBuilder):

    def __init__(self,
                 root,
                 runner,
                 app: ThirdrealityApp = ThirdrealityApp.LIGHT,
                 board: ThirdrealityBoard = ThirdrealityBoard.BL706_IoT_DVK,
        ...
        super(ThirdrealityBuilder, self).__init__(
            root=os.path.join(root, 'examples',
                              app.ExampleName(), 'thirdreality', bouffalo_chip),
            runner=runner
        )
